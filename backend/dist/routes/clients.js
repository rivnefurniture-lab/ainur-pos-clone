 ...
 */
router.get('/:companyId/clients', auth_1.isAuthenticated, async (req, res) => {
    try {
        ...
        const customersResult = await database_1.default.query(`
      SELECT *
      FROM customers
      WHERE _client = $1
      ORDER BY name
 ...
 * Get single customer
 */
router.get('/:companyId/clients/:clientId', auth_1.isAuthenticated, async (req, res) => {
    try {
        ...
        const result = await database_1.default.query('SELECT * FROM customers WHERE _id = $1 AND _client = $2', [clientId, companyId]);
 ...
 * Create new customer
 */
router.post('/:companyId/clients', auth_1.isAuthenticated, async (req, res) => {
    try {
        const { companyId } = req.params;
        const { name, type, sex, description, address, phones, emails, discount, discount_card, loyalty_type, cashback_rate } = req.body;
        const userCompanyId = (0, auth_1.getCompanyId)(req);
        if (companyId !== userCompanyId) {
            return res.status(403).json({ status: false, error: 'Access denied' });
        }
 ...
 * Update customer
 */
router.put('/:companyId/clients/:clientId', auth_1.isAuthenticated, async (req, res) => {
    try {
        const { companyId, clientId } = req.params;
        const { name, type, sex, description, address, phones, emails, discount, discount_card, loyalty_type, cashback_rate, deleted } = req.body;
        const userCompanyId = (0, auth_1.getCompanyId)(req);
        if (companyId !== userCompanyId) {
            return res.status(403).json({ status: false, error: 'Access denied' });
        }
