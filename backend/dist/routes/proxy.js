 ...
 */
router.all('/', auth_1.isAuthenticated, async (req, res) => {
    try {
        ...
        const sessionCompanyId = (0, auth_1.getCompanyId)(req);
...
async function handleDataRequest(req, res, companyId, resource, pathParts, offset, limit) {
    try {
        switch (resource) {
            case 'catalog': {
                ...
                // Get products
                const result = await database_1.default.query('SELECT * FROM products WHERE _client = $1 ORDER BY updated DESC NULLS LAST OFFSET $2 LIMIT $3', [companyId, offset, limit]);
            ...
            case 'clients': {
                const result = await database_1.default.query('SELECT * FROM customers WHERE _client = $1 ORDER BY name OFFSET $2 LIMIT $3', [companyId, offset, limit]);
                const countResult = await database_1.default.query('SELECT COUNT(*) as total FROM customers WHERE _client = $1', [companyId]);
                return res.json({ status: true, error: null, objects: result.rows.length, total: parseInt(countResult.rows[0].total), data: result.rows });
            }
            case 'stores': {
                const result = await database_1.default.query('SELECT * FROM stores WHERE _client = $1 ORDER BY name', [companyId]);
                return res.json({ status: true, error: null, objects: result.rows.length, total: result.rows.length, data: result.rows });
            }
            case 'accounts': {
                const result = await database_1.default.query('SELECT * FROM accounts WHERE _client = $1 ORDER BY name', [companyId]);
                return res.json({ status: true, error: null, objects: result.rows.length, total: result.rows.length, data: result.rows });
            }
            case 'suppliers': {
                const result = await database_1.default.query('SELECT * FROM suppliers WHERE _client = $1 ORDER BY name', [companyId]);
                return res.json({ status: true, error: null, objects: result.rows.length, total: result.rows.length, data: result.rows });
            }
            case 'sources': {
                const result = await database_1.default.query('SELECT * FROM money_sources ORDER BY title');
                return res.json({ status: true, error: null, objects: result.rows.length, total: result.rows.length, data: result.rows });
            }
            case 'registers': {
                const result = await database_1.default.query('SELECT * FROM registers WHERE _client = $1 ORDER BY name', [companyId]);
                return res.json({ status: true, error: null, objects: result.rows.length, total: result.rows.length, data: result.rows });
            }
            default:
...
async function handleCountRequest(req, res, companyId, resource) {
    try {
        let table;
        switch (resource) {
            case 'catalog':
        ...
        const result = await database_1.default.query(`SELECT COUNT(*) as total FROM ${table} WHERE _client = $1`, [companyId]);
...
async function handleSearchRequest(req, res, companyId, resource, pathParts) {
    try {
